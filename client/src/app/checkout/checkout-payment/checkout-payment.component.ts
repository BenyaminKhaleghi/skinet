...
export class CheckoutPaymentComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    this.strip = Stripe('');
    const elements = this.strip.elements();
  ...
  ngOnDestroy(): void {
    this.cardNumber.distroy();
    this.cardExpiry.distroy();
    this.cardCvc.distroy();
  }
 ...
 async submitOrder() {
  ...
  private async confirmPaymentWithStripe(basket) {
    return this.strip.confirmCardPayment(basket.clientSecret, {
        payment_method: {
